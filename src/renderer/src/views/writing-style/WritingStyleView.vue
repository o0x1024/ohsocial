...
const { styles, loadWritingStyles } = useWritingStyles()
...
<template>
  <div class="flex-1 overflow-y-auto p-6">
    ...
    <dialog :class="['modal', { 'modal-open': showForm }]">
      <div class="modal-box max-w-2xl">
        <h3 class="font-bold text-lg mb-4">{{ formTitle }}</h3>
        <div class="space-y-3">
          <input v-model="form.name" type="text" class="input input-bordered w-full" placeholder="文风名称，如：口语化科普" />
          <input v-model="form.description" type="text" class="input input-bordered w-full" placeholder="简短说明（可选）" />
          <div>
            <label class="text-sm text-base-content/60 mb-1 block">文风 Prompt</label>
            <textarea
              v-model="form.promptTemplate"
              class="textarea textarea-bordered min-h-64 w-full font-mono text-sm"
              placeholder="描述语气、句式、用词、结构等要求…"
            />
          </div>
          <div>
            <label class="text-sm text-base-content/60 mb-1 block">参考范文（可选）</label>
            <textarea
              v-model="form.referenceText"
              class="textarea textarea-bordered min-h-24 w-full text-sm"
              placeholder="AI 将模仿其表达方式"
            />
          </div>
